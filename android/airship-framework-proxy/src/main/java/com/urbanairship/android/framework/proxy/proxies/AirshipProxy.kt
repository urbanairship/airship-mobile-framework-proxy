/* Copyright Urban Airship and Contributors */package com.urbanairship.reactnativeimport
 ...
 */
public class AirshipProxy(private val context: Context) {

    private val proxyStore = ProxyStore.shared(context)

    public fun takeOff(config: JsonMap?): Boolean {
    ...
    fun setAndroidNotificationConfig(map: ReadableMap) {
        preferences.notificationIcon =
            if (map.hasKey(NOTIFICATION_ICON_KEY)) map.getString(NOTIFICATION_ICON_KEY) else null
        preferences.notificationLargeIcon =
            if (map.hasKey(NOTIFICATION_LARGE_ICON_KEY)) map.getString(NOTIFICATION_LARGE_ICON_KEY) else null
        preferences.notificationAccentColor =
            if (map.hasKey(ACCENT_COLOR_KEY)) map.getString(ACCENT_COLOR_KEY) else null
        preferences.defaultNotificationChannelId =
            if (map.hasKey(DEFAULT_CHANNEL_ID_KEY)) map.getString(DEFAULT_CHANNEL_ID_KEY) else null
    }
     ...
     * @param promise The JS promise.
     */
    public fun getActiveNotifications(promise: Promise) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            val statusBarNotifications =
                notificationManager.activeNotifications            for (statusBarNotification in statusBarNotifications) {
                val id = statusBarNotification.id
                val tag = statusBarNotification.tag
                var pushMessage: PushMessage
                val extras = statusBarNotification.notification.extras
                val bundle =
                    extras?.getBundle("push_message") pushMessage = bundle ?. let { PushMessage(it) }
                        ?: PushMessage(Bundle()) notifications . pushMap (Utils.pushPayload(
                            pushMessage,
                            id,
                            tag
                        ))
